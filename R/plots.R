...
plot.Mdata <- function(
  ...
  plot(
    ts(c(x$x, rep(NA, x$h)), end = tsp(x$x)[2] + x$h / freq, frequency = freq),
    ylim = ylim,
    xlim = xlim,
    ylab = ylab,
    xlab = xlab,
  ...
  lines(
    ts(x$xx, start = tsp(x$x)[2] + 1 / freq, frequency = freq),
    lt = 1,
    col = 2
  )
}
...
autoplot.Mdata <- function(object, ...) {
  ...
  df <- cbind(
    Training = ts(
      c(object$x, rep(NA, object$h)),
      end = tsp(object$x)[2] + object$h / freq,
      frequency = freq
    ),
